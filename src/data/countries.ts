...
COUNTRIES.forEach(c => {
  countryMap.set(c.name.toLowerCase(), c.flag);
  countryMap.set(c.code.toLowerCase(), c.flag);
});
...
export const getCountryFlag = (countryInput?: string): string => {
  ...
  // If the input already starts with an emoji flag
  ...
  // Check map
  const match = countryMap.get(trimmed.toLowerCase());
