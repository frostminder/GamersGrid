...
const oldMediaPreview = /mediaPreview/g;
file = file.replace(/postType, mediaPreview, videoFile/g, 'postType, videoPreview, videoFile');

const oldCompressAndProcessMedia = /const compressAndProcessMedia = async \(\): Promise<\{ mediaUrl: string; thumbnailUrl: string; duration: string \}> => \{[\s\S]*?\} else if \(imageFile && mediaPreview\) \{[\s\S]*?\} else \{\s*reject\(new Error\("No media provided"\)\);\s*\}\s*\}\s*\);\s*\};/g;

file = file.replace(oldCompressAndProcessMedia, `const compressAndProcessMedia = async (): Promise<{ mediaUrl: string; thumbnailUrl: string; duration: string; imageUrls?: string[] }> => {
      ...
      try {
        if (postType === 'clip' && videoFile) {
          ...
          setTimeout(() => {
            setIsProcessingMedia(false);
            resolve({
              mediaUrl: compressedImages[0], // fallback for old code
