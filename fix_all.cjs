...
// fix TS2339 for type on file
file = file.replace(/const files = Array\.from\(e\.target\.files \|\| \[\]\);/, 'const files = e.target.files ? Array.from(e.target.files) : [];');

// fix compressAndProcessMedia
const oldCompressRegex = /const compressAndProcessMedia = async \(\): Promise<\{ mediaUrl: string; thumbnailUrl: string; duration: string \}> => \{[\s\S]*?setIsProcessingMedia\(false\);\s*reject\(new Error\("No media provided"\)\);\s*\}\s*\} catch \(err\) \{/g;
file = file.replace(oldCompressRegex, `const compressAndProcessMedia = async (): Promise<{ mediaUrl: string; thumbnailUrl: string; duration: string; imageUrls?: string[] }> => {
...
// fix remaining setMediaPreview and setImageFile in handles
file = file.replace(/setMediaPreview\(null\);/g, 'setVideoPreview(null); setImagePreviews([]);');
file = file.replace(/setImageFile\(null\);/g, 'setImageFiles([]);');
